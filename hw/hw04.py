...
def reverse_recursive(s):
    ...
    def rev(l, i):
        if rest(l) == empty:
            return link(first(s))
        return link(deepest(s, i), rev(rest(l), i+1))
...
def same_shape(t1, t2):
    ...
    for branch1, branch2 in zip(branches(t1), branches(t2)):
        if not same_shape(branch1, branch2):
            return False
    return True
...
def add_trees(t1, t2):
    ...
    for branch1, branch2 in zip(branches(t1), branches(t2)):
        new_tree.append(add_trees(branch1, branch2))
    return new_tree
...
def balanced(m):
    ...
    if is_weight(m):
        ...
        return size_left * length(left) == size_right * length(right) and balanced(next_left) and balanced(next_right)
