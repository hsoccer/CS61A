...
class Frame:
    ...
    def make_child_frame(self, formals, vals):
        ...
        while formals is not nil:
            if vals is nil:
                raise SchemeError
            child.define(formals.first, complete_eval(vals.first))
...
class Procedure:
    ...
    def eval_call(self, operands, env):
        ...
        # BEGIN PROBLEM 5
        return scheme_apply(self, operands.map(lambda expr: scheme_eval(expr, env)), env)
        # END PROBLEM 5
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        expr_lambda = Pair('lambda', Pair(target.second, expressions.second))
        return do_define_form(Pair(target.first, Pair(expr_lambda, nil)), env)
        # END PROBLEM 10
    else:
...
def do_lambda_form(expressions, env):
    ...
    lambda_function = LambdaProcedure(formals, body, env)
...
def do_and_form(expressions, env):
    ...
    if expressions.second is nil:
        return scheme_eval(expressions.first, env, True)
    eval_part = scheme_eval(expressions.first, env)
    if eval_part is False:
        return False
...
def make_let_frame(bindings, env):
    ...
    # BEGIN PROBLEM 15
    def helper_let_frame(binding):
        if binding is nil:
            return nil, nil
        check_form(binding.first, 2, 2)
        formal = binding.first.first
        val = binding.first.second
        rest_formal, rest_val = helper_let_frame(binding.second)
        return Pair(formal, rest_formal), Pair(eval_all(val, env), rest_val)
    formals, vals = helper_let_frame(bindings)
    check_formals(formals)
...
def scheme_call_cc(function, env):
    ...
    body = function.body
    arg = formals.first
    env.define(arg, ContinuationProcedure(ContinuationError(arg)))
    # END PROBLEM 21
...
def do_mu_form(expressions, env):
    ...
    # BEGIN PROBLEM 16
    body = expressions.second
    mu_function = MuProcedure(formals, body)
    return mu_function
    # END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
    ...
    # Evaluate atoms
    if scheme_symbolp(expr):
        return complete_eval(env.lookup(expr))
    elif self_evaluating(expr):
        return expr
