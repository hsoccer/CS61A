...
def remove_all(link , value):
    ...
    <0 1>
    """
    if link is Link.empty or link.rest is not Link.empty:
...
def reverse_other(t):
    ...
    def helper(tree, i):
        if i % 2 == 0:
            """reverse children"""
            root_lst = [b.root for b in tree.branches]
            root_lst.reverse()
            for j in range(len(tree.branches)):
                tree.branches[j].root = root_lst[j]
